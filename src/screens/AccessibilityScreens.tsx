      ...
      <div className="w-48 h-48 relative flex items-center justify-center">
        ...
        <div className="relative z-10 flex flex-col items-center gap-3">
          <div className="w-44 h-11 bg-white rounded-2xl flex items-center gap-2 px-3 shadow-md border border-[var(--border)]">
            ...
            <span className="text-xs text-[var(--muted)]">{tx("할랄 음식 검색...")}</span>
          </div>
          <div className="flex gap-2">
            {tx(["한식", "터키", "우즈벡", "인도"].map((c) => (
              <div key={c} className="bg-white rounded-full px-2.5 py-1 text-[10px] font-medium text-[#1A1A18] shadow-sm">{tx(c)}</div>
            )))}
          </div>
          <div className="flex gap-2">
      ...
      <div className="w-48 h-48 relative flex items-center justify-center">
        ...
        <div className="relative z-10 flex flex-col items-center gap-3">
          <div className="rounded-xl p-3 flex items-center gap-2 shadow-md" style={{ background: "linear-gradient(135deg, var(--green-dark), var(--green))" }}>
            ...
            <div>
              <p className="text-white text-xs font-semibold">{tx("다음 기도: 아스르 Asr")}</p>
          ...
          <div className="flex gap-2">
            <div className="bg-white rounded-xl px-3 py-2 shadow-sm text-center">
              ...
              <p className="text-[10px] font-bold text-[#1A1A18] mt-0.5">{tx("서울 중앙")}</p>
              <p className="text-[9px] text-[var(--muted)]">0.8km</p>
            </div>
            <div className="bg-white rounded-xl px-3 py-2 shadow-sm text-center">
              <span className="text-lg">🧭</span>
              <p className="text-[10px] font-bold text-[#1A1A18] mt-0.5">{tx("키블라")}</p>
      ...
      <div className="w-48 h-48 relative flex items-center justify-center">
        ...
        <div className="relative z-10 flex flex-col items-center gap-3">
          ...
          <div className="flex items-center gap-2 bg-white rounded-xl px-3 py-2 shadow-md">
            ...
            <p className="text-xs font-bold text-[#1A1A18]">{tx("할랄 인증 확인됨")}</p>
          </div>
        </div>
      </div>
    ),
          ...
          <div className="bg-white rounded-2xl shadow-md px-4 py-3 flex gap-4">
            {tx([
              ...
              <div key={tab.label} className="flex flex-col items-center gap-0.5">
                <span className="text-lg" style={{ opacity: tab.active ? 1 : 0.4 }}>{tx(tab.icon)}</span>
                <span className="text-[9px] font-medium" style={{ color: tab.active ? "var(--green)" : "var(--muted)" }}>{tx(tab.label)}</span>
                {tx(tab.active && <div className="w-1 h-1 rounded-full" style={{ backgroundColor: "var(--green)" }} />)}
              </div>
            )))}
          </div>
          <div className="flex gap-2">
            {tx(["식당 목록", "기도 시간", "주문 내역", "내 프로필"].map((label) => (
              <div key={label} className="bg-white rounded-lg px-2 py-1 shadow-sm">
                <p className="text-[9px] font-medium text-[var(--muted)]">{tx(label)}</p>
              </div>
            )))}
          </div>
        </div>
...
export const TutorialScreen = ({ onNavigate }: { onNavigate?: (s: ScreenId) => void }) => {
  const { t } = useLanguage();
  const tutorialSteps = makeTutorialSteps();
  ...
  return (
    <div className="flex flex-col h-full bg-white">
      ...
      <div className="flex items-center justify-between px-5 pt-2 pb-1">
        <p className="text-xs font-medium text-[var(--muted)]">{tx(step + 1)} / {tx(tutorialSteps.length)}</p>
        <button onClick={() => onNavigate?.("home")} className="text-sm font-medium" style={{ color: "var(--muted)" }}>{tx("건너뛰기")}</button>
      </div>
      ...
      <div className="flex-1 flex flex-col items-center justify-center px-8 gap-6">
        ...
        <div className="text-center space-y-2">
          <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-bold text-white" style={{ backgroundColor: current.color }}>
            <span>{tx(current.icon)}</span>
            <span>{tx(current.title)}</span>
          </div>
          <p className="text-sm text-[#6B7280] leading-relaxed whitespace-pre-line mt-3">{tx(current.body)}</p>
        </div>

        {/* Dots */}
        <div className="flex gap-2">
          {tx(tutorialSteps.map((_, i) => (
            <button
              key={i}
              onClick={() => setStep(i)}
...
const menuItems = [
  {
    ...
    uz: { name: "Halol qovurg‘a sho‘rvasi", desc: "Suyak bulyonida mol qovurg‘asi sho‘rvasi", price: "₩13,500", rtl: false },
  },
  {
    ...
    uz: { name: "Bibimbap (halol)", desc: "Sabzavotlar va halol mol go‘shtli guruch", price: "₩11,000", rtl: false },
  },
  {
    ...
    uz: { name: "Soya sho‘rvasi to‘plami", desc: "Soya sho‘rvasi, guruch va 3 ta garnir", price: "₩12,000", rtl: false },
  },
];
...
const langs = [
  ...
  { code: "uz", label: "O‘zbek", flag: "🇺🇿", dir: "ltr" as const },
];

export const MultilingualScreen = ({ onNavigate }: { onNavigate?: (s: ScreenId) => void }) => {
  const { t } = useLanguage();
  const [activePreview, setActivePreview] = useState<"split" | "ko" | "en" | "uz">("split");
      ...
      <div className="bg-white border-b border-[var(--border)] flex-shrink-0">
        ...
        <div className="flex items-center gap-3 px-4 pb-3">
          ...
          <div className="flex-1">
            ...
            <p className="text-[11px] text-[var(--muted)]">{tx("한국어 · English · O‘zbek")}</p>
          </div>
        </div>
        ...
        <div className="relative">
          ...
          <div className="absolute bottom-3 left-4 right-4">
            <p className="text-white font-bold text-base">
              {tx(activePreview === "uz" ? "Sindang Halal Kitchen" : activePreview === "en" ? "Sindang Halal Korean Kitchen" : "신당 할랄 키친")}
            </p>
            <div className="flex items-center gap-2 mt-0.5">
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-full text-white" style={{ backgroundColor: "var(--green)" }}>
                {tx(activePreview === "uz" ? "Halol sertifikatlangan" : activePreview === "en" ? "HALAL CERTIFIED" : "HALAL 인증")}
              </span>
              <span className="text-white/70 text-xs">⭐ 4.8 · {tx(activePreview === "uz" ? "2.3 km" : "2.3km")}</span>
            </div>
          </div>
        </div>
        ...
        <div className="px-4 pt-4 space-y-3">
          ...
          <div className="flex items-center justify-between">
            <p className="font-bold text-sm text-[#1A1A18]">
              {tx(activePreview === "uz" ? "🔥 Ommabop menyu" : activePreview === "en" ? "🔥 Popular Menu" : "🔥 인기메뉴")}
            </p>
          </div>

          {tx(menuItems.map((item, i) => {
                      ...
                      return (
                        <div key={lang.code} className="p-2.5 min-h-[80px]" dir={content.rtl ? "rtl" : "ltr"}>
                          <div className="flex items-center gap-1 mb-1.5">
                            <span className="text-[10px]">{tx(lang.flag)}</span>
                            <span className="text-[9px] font-bold text-[var(--muted)] uppercase">{tx(lang.label)}</span>
                          </div>
                          <p className={`font-bold text-[11px] text-[#1A1A18] leading-tight mb-1 ${lang.code === "uz" ? "" : ""}`}>
                            {tx(content.name)}
                          </p>
                          <p className={`text-[9px] text-[var(--muted)] leading-relaxed line-clamp-2 ${lang.code === "uz" ? "" : ""}`}>
                            {tx(content.desc)}
                          </p>
                          <p className="font-bold text-[10px] text-[#1A1A18] mt-1">{tx(content.price)}</p>
                        </div>
                      );
                    }))}
                  </div>
            ...
            return (
              <div key={i} className="bg-white rounded-2xl p-3 shadow-sm flex gap-3" dir={content.rtl ? "rtl" : "ltr"}>
                ...
                <div className="flex-1">
                  <p className={`font-bold text-sm text-[#1A1A18] ${lang.code === "uz" ? "" : ""}`}>{tx(content.name)}</p>
                  <p className={`text-xs text-[var(--muted)] mt-0.5 leading-relaxed ${lang.code === "uz" ? "" : ""}`}>{tx(content.desc)}</p>
                  <div className="flex items-center justify-between mt-2">
                    <p className="font-bold text-sm text-[#1A1A18]">{tx(content.price)}</p>
                    <button type="button" onClick={() => navigateTo("item-detail?restaurant=sindang-halal&item=m" + (i + 1))} className="w-8 h-8 rounded-full flex items-center justify-center text-white font-bold text-lg" style={{ backgroundColor: "var(--green)" }}>+</button>
                  </div>
                </div>
              </div>
            );
